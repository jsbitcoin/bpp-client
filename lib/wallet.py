...
import Queue
import time
import deterministic as Btclib
...
class Wallet:
    ...
    def import_key_bip32(self, address, path, password):
        # check password
        seed = self.get_seed(password)
        if self.is_mine(address):
            raise Exception('Address already in wallet')
        ...
        # store the originally requested keypair into the imported keys table
        self.imported_keys[address] = pw_encode(path, password)
        self.storage.put('imported_keys', self.imported_keys, True)
        if self.synchronizer:
            self.synchronizer.subscribe_to_addresses([address])
        return address
    ...
    def get_private_key(self, address, password):
        ...
        out = []
        if address in self.imported_keys.keys():
	    if '/' in self.imported_keys[address]:
		p1, p2, p3 = pw_decode(self.imported_keys[address], password).split('/')
		out.append(Btclib.bip32_extract_key(Btclib.subkey_for_path(self.privbtc, p1+'/'+p2+'/'+p3), 'wif'))
	    else:
